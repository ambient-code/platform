...
build-all: build-frontend build-backend build-operator build-runner build-state-sync build-public-api build-api-server build-control-plane build-mcp build-observability-dashboard ## Build all container images
...
push-all: registry-login ## Push all images to registry
	...
	@for image in $(FRONTEND_IMAGE) $(BACKEND_IMAGE) $(OPERATOR_IMAGE) $(RUNNER_IMAGE) $(STATE_SYNC_IMAGE) $(PUBLIC_API_IMAGE) $(API_SERVER_IMAGE) $(CONTROL_PLANE_IMAGE) $(MCP_IMAGE) $(OBSERVABILITY_DASHBOARD_IMAGE); do \
...
_kind-load-images: ## Internal: Load images into kind cluster
	...
	@for img in $(BACKEND_IMAGE) $(FRONTEND_IMAGE) $(OPERATOR_IMAGE) $(RUNNER_IMAGE) $(STATE_SYNC_IMAGE) $(PUBLIC_API_IMAGE) $(API_SERVER_IMAGE) $(MCP_IMAGE) $(OBSERVABILITY_DASHBOARD_IMAGE); do \
