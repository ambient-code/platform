...
test-colors: ## Test color output rendering
	@printf "%b\n" "$(COLOR_BOLD)Testing Color Output$(COLOR_RESET)"
	@printf "%b\n" ""
	@printf "%b\n" "$(COLOR_RED)✗ Red text (errors)$(COLOR_RESET)"
	@printf "%b\n" "$(COLOR_GREEN)✓ Green text (success)$(COLOR_RESET)"
	@printf "%b\n" "$(COLOR_BLUE)▶ Blue text (info)$(COLOR_RESET)"
	@printf "%b\n" "$(COLOR_YELLOW)⚠ Yellow text (warnings)$(COLOR_RESET)"
	@printf "%b\n" "$(COLOR_BOLD)Bold text$(COLOR_RESET)"
	@printf "%b\n" ""
	@printf "%b\n" "If you see color codes like \033[1m instead of colors, your terminal may not support ANSI colors"

help: ## Display this help message
	@printf "%b\n" "$(COLOR_BOLD)Ambient Code Platform - Development Makefile$(COLOR_RESET)"
	@printf "%b\n" ""
	@printf "%b\n" "$(COLOR_BOLD)Quick Start:$(COLOR_RESET)"
	@printf "%b\n" "  $(COLOR_GREEN)make local-up$(COLOR_RESET)            Start local development environment"
	@printf "%b\n" "  $(COLOR_GREEN)make local-status$(COLOR_RESET)        Check status of local environment"
	@printf "%b\n" "  $(COLOR_GREEN)make local-logs$(COLOR_RESET)          View logs from all components"
	@printf "%b\n" "  $(COLOR_GREEN)make local-down$(COLOR_RESET)          Stop local environment"
	@printf "%b\n" ""
	@printf "%b\n" "$(COLOR_BOLD)Quality Assurance:$(COLOR_RESET)"
	@printf "%b\n" "  $(COLOR_GREEN)make validate-makefile$(COLOR_RESET)   Validate Makefile quality (runs in CI)"
	@printf "%b\n" "  $(COLOR_GREEN)make makefile-health$(COLOR_RESET)     Run comprehensive health check"
	@printf "%b\n" ""
	@awk 'BEGIN {FS = ":.*##"; printf "$(COLOR_BOLD)Available Targets:$(COLOR_RESET)\n"} /^[a-zA-Z_-]+:.*?##/ { printf "  $(COLOR_BLUE)%-20s$(COLOR_RESET) %s\n", $$1, $$2 } /^##@/ { printf "\n$(COLOR_BOLD)%s$(COLOR_RESET)\n", substr($$0, 5) } ' $(MAKEFILE_LIST)
	@printf "%b\n" ""
	@printf "%b\n" "$(COLOR_BOLD)Configuration Variables:$(COLOR_RESET)"
	@printf "%b\n" "  CONTAINER_ENGINE=$(CONTAINER_ENGINE)  (docker or podman)"
	@printf "%b\n" "  NAMESPACE=$(NAMESPACE)"
	@printf "%b\n" "  PLATFORM=$(PLATFORM)"
	@printf "%b\n" ""
...
remove-hooks: ## Remove git hooks
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Removing git hooks..."
	@rm -f .git/hooks/pre-commit
	@rm -f .git/hooks/pre-push
	@printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) Git hooks removed"

##@ Registry Operations

registry-login: ## Login to container registry
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Logging in to $(REGISTRY)..."
	@$(CONTAINER_ENGINE) login $(REGISTRY)

push-all: registry-login ## Push all images to registry
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Pushing images to $(REGISTRY)..."
	...
	@printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) All images pushed"

##@ Local Development (Minikube)

local-up: check-minikube check-kubectl ## Start local development environment (minikube)
	@printf "%b\n" "$(COLOR_BOLD)🚀 Starting Ambient Code Platform Local Environment$(COLOR_RESET)"
	@printf "%b\n" ""
	...
	@printf "%b\n" "  • Wait ~30s for pods to be ready"
	@printf "%b\n" "  • Run: $(COLOR_BOLD)make local-status$(COLOR_RESET) to check deployment"
	@printf "%b\n" "  • Run: $(COLOR_BOLD)make local-logs$(COLOR_RESET) to view logs"

local-down: check-kubectl ## Stop Ambient Code Platform (keep minikube running)
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Stopping Ambient Code Platform..."
	@$(MAKE) --no-print-directory local-stop-port-forward
	@kubectl delete namespace $(NAMESPACE) --ignore-not-found=true --timeout=60s
	@printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) Ambient Code Platform stopped (minikube still running)"
	@printf "%b\n" "  To stop minikube: $(COLOR_BOLD)make local-clean$(COLOR_RESET)"

local-clean: check-minikube ## Delete minikube cluster completely
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Deleting minikube cluster..."
	@$(MAKE) --no-print-directory local-stop-port-forward
	@minikube delete
	@printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) Minikube cluster deleted"

local-status: check-kubectl ## Show status of local deployment
	@printf "%b\n" "$(COLOR_BOLD)📊 Ambient Code Platform Status$(COLOR_RESET)"
	@printf "%b\n" ""
	@printf "%b\n" "$(COLOR_BOLD)Minikube:$(COLOR_RESET)"
	@minikube status 2>/dev/null || printf "%b\n" "$(COLOR_RED)✗$(COLOR_RESET) Minikube not running"
	@printf "%b\n" ""
	@printf "%b\n" "$(COLOR_BOLD)Pods:$(COLOR_RESET)"
	@kubectl get pods -n $(NAMESPACE) -o wide 2>/dev/null || printf "%b\n" "$(COLOR_RED)✗$(COLOR_RESET) Namespace not found"
	@printf "%b\n" ""
	@printf "%b\n" "$(COLOR_BOLD)Services:$(COLOR_RESET)"
	@kubectl get svc -n $(NAMESPACE) 2>/dev/null | grep -E "NAME|NodePort" || printf "%b\n" "No services found"
	@printf "%b\n" ""
	@$(MAKE) --no-print-directory _show-access-info
	@printf "%b\n" ""
	@printf "%b\n" "$(COLOR_BOLD)Version Status:$(COLOR_RESET)"
	...
	if [ "$$GIT_VERSION" != "$$MANIFEST_VERSION" ]; then \
	  printf "%b\n" "  $(COLOR_YELLOW)⚠$(COLOR_RESET)  Manifest version differs from git (run 'make local-sync-version')"; \
	fi

local-sync-version: ## Sync version from git to local deployment manifests
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Syncing version from git..."
	...
	printf "%b\n" "  $(COLOR_GREEN)✓$(COLOR_RESET) Version synced to $$VERSION"

local-rebuild: ## Rebuild and reload all components
	@printf "%b\n" "$(COLOR_BOLD)🔄 Rebuilding all components...$(COLOR_RESET)"
	@$(MAKE) --no-print-directory _build-and-load
	@$(MAKE) --no-print-directory _restart-all
	@printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) All components rebuilt and reloaded"

local-reload-backend: ## Rebuild and reload backend only
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Rebuilding backend..."
	...
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Restarting backend..."
	...
	@printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) Backend reloaded"
	@OS=$$(uname -s); \
	if [ "$$OS" = "Darwin" ] && [ "$(CONTAINER_ENGINE)" = "podman" ]; then \
		printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Restarting backend port forward..."; \
...
local-reload-frontend: ## Rebuild and reload frontend only
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Rebuilding frontend..."
	...
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Restarting frontend..."
	@kubectl rollout restart deployment/frontend -n $(NAMESPACE) >/dev/null 2>&1
	@kubectl rollout status deployment/frontend -n $(NAMESPACE) --timeout=60s
	@printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) Frontend reloaded"
	@OS=$$(uname -s); \
	if [ "$$OS" = "Darwin" ] && [ "$(CONTAINER_ENGINE)" = "podman" ]; then \
		printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Restarting frontend port forward..."; \
...
local-reload-operator: ## Rebuild and reload operator only
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Rebuilding operator..."
	...
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Restarting operator..."
	...
	@printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) Operator reloaded"
...
validate-makefile: lint-makefile check-shell ## Validate Makefile quality and syntax
	@printf "%b\n" "$(COLOR_GREEN)✓ Makefile validation passed$(COLOR_RESET)"

lint-makefile: ## Lint Makefile for syntax and best practices
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Linting Makefile..."
	...
	if [ -n "$$missing_help" ]; then \
		printf "%b\n" "$(COLOR_YELLOW)⚠$(COLOR_RESET)  Targets missing help text:"; \
		echo "$$missing_help" | head -5; \
	fi
	@# Check for common mistakes
	@if grep -n "^\t " $(MAKEFILE_LIST) | grep -v "^#" >/dev/null 2>&1; then \
		printf "%b\n" "$(COLOR_RED)✗$(COLOR_RESET) Found tabs followed by spaces (use tabs only for indentation)"; \
	...
	@if grep -E '\$$[^($$@%<^+?*]' $(MAKEFILE_LIST) | grep -v "^#" | grep -v '\$$\$$' >/dev/null 2>&1; then \
		printf "%b\n" "$(COLOR_YELLOW)⚠$(COLOR_RESET)  Possible unprotected variable references found"; \
	fi
	@# Verify .PHONY declarations exist
	@if ! grep -q "^\.PHONY:" $(MAKEFILE_LIST); then \
		printf "%b\n" "$(COLOR_RED)✗$(COLOR_RESET) No .PHONY declarations found"; \
		exit 1; \
	fi
	@printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) Makefile syntax validated"

check-shell: ## Validate shell scripts with shellcheck (if available)
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Checking shell scripts..."
	@if command -v shellcheck >/dev/null 2>&1; then \
		echo "  Running shellcheck on test scripts..."; \
		shellcheck tests/local-dev-test.sh 2>/dev/null || printf "%b\n" "$(COLOR_YELLOW)⚠$(COLOR_RESET)  shellcheck warnings in tests/local-dev-test.sh"; \
		if [ -d e2e/scripts ]; then \
			shellcheck e2e/scripts/*.sh 2>/dev/null || printf "%b\n" "$(COLOR_YELLOW)⚠$(COLOR_RESET)  shellcheck warnings in e2e scripts"; \
		fi; \
		printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) Shell scripts checked"; \
	else \
		printf "%b\n" "$(COLOR_YELLOW)⚠$(COLOR_RESET)  shellcheck not installed (optional)"; \
...
makefile-health: check-minikube check-kubectl ## Run comprehensive Makefile health check
	@printf "%b\n" "$(COLOR_BOLD)🏥 Makefile Health Check$(COLOR_RESET)"
	@echo ""
	@printf "%b\n" "$(COLOR_BOLD)Prerequisites:$(COLOR_RESET)"
	@minikube version >/dev/null 2>&1 && printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) minikube available" || printf "%b\n" "$(COLOR_RED)✗$(COLOR_RESET) minikube missing"
	@kubectl version --client >/dev/null 2>&1 && printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) kubectl available" || printf "%b\n" "$(COLOR_RED)✗$(COLOR_RESET) kubectl missing"
	@command -v $(CONTAINER_ENGINE) >/dev/null 2>&1 && printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) $(CONTAINER_ENGINE) available" || printf "%b\n" "$(COLOR_RED)✗$(COLOR_RESET) $(CONTAINER_ENGINE) missing"
	@echo ""
	...
	@echo ""
	@printf "%b\n" "$(COLOR_GREEN)✓ Makefile health check complete$(COLOR_RESET)"

local-test-dev: ## Run local developer experience tests
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Running local developer experience tests..."
...
local-test-quick: check-kubectl check-minikube ## Quick smoke test of local environment
	...
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Testing pods..."
	@kubectl get pods -n $(NAMESPACE) 2>/dev/null | grep -q "Running" && printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) Pods running" || (printf "%b\n" "$(COLOR_RED)✗$(COLOR_RESET) No pods running" && exit 1)
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Testing backend health..."
	@curl -sf http://$$(minikube ip):30080/health >/dev/null 2>&1 && printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) Backend healthy" || (printf "%b\n" "$(COLOR_RED)✗$(COLOR_RESET) Backend not responding" && exit 1)
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Testing frontend..."
	@curl -sf http://$$(minikube ip):30030 >/dev/null 2>&1 && printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) Frontend accessible" || (printf "%b\n" "$(COLOR_RED)✗$(COLOR_RESET) Frontend not responding" && exit 1)
	@echo ""
	@printf "%b\n" "$(COLOR_GREEN)✓ Quick smoke test passed!$(COLOR_RESET)"
...
local-logs: check-kubectl ## Show logs from all components (follow mode)
	@printf "%b\n" "$(COLOR_BOLD)📋 Streaming logs from all components (Ctrl+C to stop)$(COLOR_RESET)"
	@kubectl logs -n $(NAMESPACE) -l 'app in (backend-api,frontend,agentic-operator)' --tail=20 --prefix=true -f 2>/dev/null || \
		printf "%b\n" "$(COLOR_RED)✗$(COLOR_RESET) No pods found. Run 'make local-status' to check deployment."
...
local-shell: check-kubectl ## Open shell in backend pod
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Opening shell in backend pod..."
	@kubectl exec -it -n $(NAMESPACE) $$(kubectl get pod -n $(NAMESPACE) -l app=backend-api -o jsonpath='{.items[0].metadata.name}' 2>/dev/null) -- /bin/sh 2>/dev/null || \
		printf "%b\n" "$(COLOR_RED)✗$(COLOR_RESET) Backend pod not found or not ready"

local-shell-frontend: check-kubectl ## Open shell in frontend pod
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Opening shell in frontend pod..."
	@kubectl exec -it -n $(NAMESPACE) $$(kubectl get pod -n $(NAMESPACE) -l app=frontend -o jsonpath='{.items[0].metadata.name}' 2>/dev/null) -- /bin/sh 2>/dev/null || \
		printf "%b\n" "$(COLOR_RED)✗$(COLOR_RESET) Frontend pod not found or not ready"
...
local-port-forward: check-kubectl ## Port-forward for direct access (8080→backend, 3000→frontend)
	@printf "%b\n" "$(COLOR_BOLD)🔌 Setting up port forwarding$(COLOR_RESET)"
	@echo ""
	@echo "  Backend:  http://localhost:8080"
	@echo "  Frontend: http://localhost:3000"
	@echo ""
	@printf "%b\n" "$(COLOR_YELLOW)Press Ctrl+C to stop$(COLOR_RESET)"
	@echo ""
	@trap 'printf "%b\n" ""; printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) Port forwarding stopped"; exit 0' INT; \
...
local-troubleshoot: check-kubectl ## Show troubleshooting information
	@printf "%b\n" "$(COLOR_BOLD)🔍 Troubleshooting Information$(COLOR_RESET)"
	@echo ""
	@printf "%b\n" "$(COLOR_BOLD)Pod Status:$(COLOR_RESET)"
	@kubectl get pods -n $(NAMESPACE) -o wide 2>/dev/null || printf "%b\n" "$(COLOR_RED)✗$(COLOR_RESET) No pods found"
	@echo ""
	@printf "%b\n" "$(COLOR_BOLD)Recent Events:$(COLOR_RESET)"
	@kubectl get events -n $(NAMESPACE) --sort-by='.lastTimestamp' | tail -10 2>/dev/null || echo "No events"
	@echo ""
	@printf "%b\n" "$(COLOR_BOLD)Failed Pods (if any):$(COLOR_RESET)"
...
deploy: ## Deploy to production Kubernetes cluster
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Deploying to Kubernetes..."
	@cd components/manifests && ./deploy.sh
	@printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) Deployment complete"

clean: ## Clean up Kubernetes resources
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Cleaning up..."
	@cd components/manifests && ./deploy.sh clean
	@printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) Cleanup complete"

##@ E2E Testing (kind-based)

e2e-test: ## Run complete e2e test suite (setup, deploy, test, cleanup)
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Running e2e tests..."
...
e2e-setup: ## Install e2e test dependencies
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Installing e2e test dependencies..."
	cd e2e && npm install

e2e-clean: ## Clean up e2e test environment
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Cleaning up e2e environment..."
...
deploy-langfuse-openshift: ## Deploy Langfuse to OpenShift/ROSA cluster
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Deploying Langfuse to OpenShift cluster..."
...
check-minikube: ## Check if minikube is installed
	@command -v minikube >/dev/null 2>&1 || \
		(printf "%b\n" "$(COLOR_RED)✗$(COLOR_RESET) minikube not found. Install: https://minikube.sigs.k8s.io/docs/start/" && exit 1)

check-kubectl: ## Check if kubectl is installed
	@command -v kubectl >/dev/null 2>&1 || \
		(printf "%b\n" "$(COLOR_RED)✗$(COLOR_RESET) kubectl not found. Install: https://kubernetes.io/docs/tasks/tools/" && exit 1)
...
_restart-all: ## Internal: Restart all deployments
	...
	@printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Waiting for deployments to be ready..."
...
_show-access-info: ## Internal: Show access information
	...
	@OS=$$(uname -s); \
	if [ "$$OS" = "Darwin" ] && [ "$(CONTAINER_ENGINE)" = "podman" ]; then \
		printf "%b\n" "  $(COLOR_YELLOW)Note:$(COLOR_RESET) Port forwarding will start automatically"; \
		echo "  Once pods are ready, access at:"; \
		printf "%b\n" "     Frontend: $(COLOR_BLUE)http://localhost:3000$(COLOR_RESET)"; \
		printf "%b\n" "     Backend:  $(COLOR_BLUE)http://localhost:8080$(COLOR_RESET)"; \
		echo ""; \
		printf "%b\n" "  $(COLOR_BOLD)To manage port forwarding:$(COLOR_RESET)"; \
		printf "%b\n" "    Stop:    $(COLOR_BOLD)make local-stop-port-forward$(COLOR_RESET)"; \
		printf "%b\n" "    Restart: $(COLOR_BOLD)make local-port-forward$(COLOR_RESET)"; \
	else \
		MINIKUBE_IP=$$(minikube ip 2>/dev/null) && \
			printf "%b\n" "  Frontend: $(COLOR_BLUE)http://$$MINIKUBE_IP:30030$(COLOR_RESET)" && \
			printf "%b\n" "  Backend:  $(COLOR_BLUE)http://$$MINIKUBE_IP:30080$(COLOR_RESET)" || \
			printf "%b\n" "  $(COLOR_RED)✗$(COLOR_RESET) Cannot get minikube IP"; \
		echo ""; \
		printf "%b\n" "$(COLOR_BOLD)Alternative:$(COLOR_RESET) Port forward for localhost access"; \
		printf "%b\n" "  Run: $(COLOR_BOLD)make local-port-forward$(COLOR_RESET)"; \
		echo "  Then access:"; \
		printf "%b\n" "    Frontend: $(COLOR_BLUE)http://localhost:3000$(COLOR_RESET)"; \
		printf "%b\n" "    Backend:  $(COLOR_BLUE)http://localhost:8080$(COLOR_RESET)"; \
	fi
...
_create-operator-config: ## Internal: Create operator config from environment variables
	...
	if [ -n "$$VERTEX_PROJECT_ID" ]; then \
		if [ -n "$$VERTEX_KEY_FILE" ] && [ -f "$$VERTEX_KEY_FILE" ]; then \
			...
			USE_VERTEX="1"; \
			AUTH_METHOD="adc"; \
			printf "%b\n" "  $(COLOR_GREEN)✓$(COLOR_RESET) Found Vertex AI config (gcloud ADC)"; \
			echo "    Project: $$VERTEX_PROJECT_ID"; \
			echo "    Region: $$CLOUD_REGION"; \
			...
			printf "%b\n" "  $(COLOR_YELLOW)⚠$(COLOR_RESET)  ANTHROPIC_VERTEX_PROJECT_ID set but no credentials found"; \
			echo "    Run: gcloud auth application-default login"; \
			echo "    Or set: GOOGLE_APPLICATION_CREDENTIALS=/path/to/key.json"; \
			echo "    Using direct Anthropic API for now"; \
		fi; \
	else \
		printf "%b\n" "  $(COLOR_YELLOW)ℹ$(COLOR_RESET)  Vertex AI not configured"; \
...
_auto-port-forward: ## Internal: Auto-start port forwarding on macOS with Podman
	...
	if [ "$$OS" = "Darwin" ] && [ "$(CONTAINER_ENGINE)" = "podman" ]; then \
		echo ""; \
		printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Starting port forwarding in background..."; \
		...
		if ps -p $$(cat /tmp/ambient-code/port-forward-backend.pid 2>/dev/null) > /dev/null 2>&1 && \
		   ps -p $$(cat /tmp/ambient-code/port-forward-frontend.pid 2>/dev/null) > /dev/null 2>&1; then \
			printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) Port forwarding started"; \
			printf "%b\n" "  $(COLOR_BOLD)Access at:$(COLOR_RESET)"; \
			printf "%b\n" "    Frontend: $(COLOR_BLUE)http://localhost:3000$(COLOR_RESET)"; \
			printf "%b\n" "    Backend:  $(COLOR_BLUE)http://localhost:8080$(COLOR_RESET)"; \
		else \
			printf "%b\n" "$(COLOR_YELLOW)⚠$(COLOR_RESET)  Port forwarding started but may need time for pods"; \
			printf "%b\n" "  If connection fails, wait for pods and run: $(COLOR_BOLD)make local-port-forward$(COLOR_RESET)"; \
		fi; \
	fi

local-stop-port-forward: ## Stop background port forwarding
	@if [ -f /tmp/ambient-code/port-forward-backend.pid ]; then \
		printf "%b\n" "$(COLOR_BLUE)▶$(COLOR_RESET) Stopping port forwarding..."; \
		...
		printf "%b\n" "$(COLOR_GREEN)✓$(COLOR_RESET) Port forwarding stopped"; \
	fi
