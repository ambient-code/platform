...
The Ambient Code Platform is a Kubernetes-native AI automation platform that combines multiple AI providers (Claude, Gemini, and future providers) with multi-agent collaboration capabilities, enabling intelligent agentic sessions through pluggable client interfaces including REST API, SDK, MCP, and web UI. The platform is MIT licensed and built on open source, standards-based technologies to enable shared implementations across providers.

### Purpose & Scope

This constitution:
- Defines non-negotiable technical principles that ensure platform quality, security, and scalability
- Establishes development standards across all components:
  - **Core Platform**: Backend, Frontend, Operator, Runner
  - **API Layer**: Public API, Ambient API Server
  - **Client Libraries**: Ambient SDK (Go, Python, TypeScript), Ambient CLI (acpctl)
  - **Integrations**: Open WebUI, MCP Server (coming), Mobile App (coming)
...
1. [Core Principles](#core-principles)
   - [I. Kubernetes-Native Architecture](#i-kubernetes-native-architecture)
   - [II. Security & Multi-Tenancy First](#ii-security--multi-tenancy-first)
   - [III. Type Safety & Error Handling](#iii-type-safety--error-handling)
   - [IV. Test-Driven Development](#iv-test-driven-development)
   - [V. Component Modularity](#v-component-modularity)
   - [VI. Observability & Monitoring](#vi-observability--monitoring)
   - [VII. Resource Lifecycle Management](#vii-resource-lifecycle-management)
   - [VIII. Context Engineering & Prompt Optimization](#viii-context-engineering--prompt-optimization)
   - [IX. Data Access & Knowledge Augmentation](#ix-data-access--knowledge-augmentation)
   - [X. Commit Discipline & Code Review](#x-commit-discipline--code-review)
   - [XI. System Dynamics & Control Theory](#xi-system-dynamics--control-theory)
   - [XII. Dependency Management](#xii-dependency-management)
...
- **Jobs and JobSets** for execution workloads
  - Jobs for single session execution
  - JobSets for coordinated multi-pod workloads
...
- **Pluggable Interfaces** for extensibility
  - Pluggable clients: REST API, SDK, MCP, Web UI (not limited to these)
  - Pluggable UIs: Support custom frontends via Public API
  - Provider-agnostic design: Support multiple AI providers (Claude, Gemini, future providers)
  - Standards-based: Use open protocols and formats for interoperability

**Rationale**: Kubernetes-native design ensures portability, scalability, and enterprise-grade operational tooling. Pluggable interfaces enable ecosystem growth without platform modification. Violations create operational complexity, reduce platform value, and prevent leveraging Kubernetes ecosystem tools.
...
- Respect provider-specific token limits (varies by model and provider)
- Track context usage during session
- Warn when approaching limits
- Implement context pruning strategies
- Design for portable context that works across providers
...
**Status**: MANDATORY
**Applies To**: All system design, architecture decisions, and process improvements

The platform is designed around system dynamics and control theory principles. Every process, workflow, and system component should be analyzed through this lens:

#### Reinforcing Loops (Flywheels)

Reinforcing loops amplify what is already happening. Growth feeds more growth. These are the loops we want to identify and accelerate:

- **Identify reinforcing loops** in every system design
- **Accelerate** existing reinforcing loops where beneficial
- **Create** new reinforcing loops that compound capability over time
- **Examples**: User feedback → improvements → more users → more feedback; automated testing → faster delivery → more features → more testing

#### Balancing Loops (Governors)

Balancing loops resist change and pull systems toward equilibrium. They keep things stuck at a lower level than desired:

- **Identify balancing loops** that limit system performance
- **Remove** or **redesign** balancing loops that constrain growth
- **Examples**: Manual code review creates bottleneck → slower delivery → fewer PRs; technical debt → slower development → less refactoring time

#### Loop States

Every loop exists in one of three states:

| State | Meaning | Action |
|-------|---------|--------|
| **Absent** | Loop does not exist yet | Decide whether to create it |
| **Present** | Loop exists at some speed | Measure and tune it |
| **Throttled** | External constraint limits speed | Find and remove constraint |

#### Design Questions

For every system, process, or workflow, ask:

1. **Where are the reinforcing loops?** What compounds here?
2. **Where are the balancing loops?** What is self-limiting here?
3. **What is the cheapest intervention** that speeds up a reinforcing loop or removes a balancing loop?

#### The Delay Factor

Reinvesting in capability takes time. The delay between investment and payoff is where most teams lose nerve. Constitution-compliant design:

- **Accepts strategic delays** for long-term compounding benefits
- **Documents expected payoff timelines** for capability investments
- **Prioritizes** reinforcing loops over balancing loop workarounds

**Rationale**: The outputs of this platform are not code—they are systems that produce code. The compounding effect of that distinction is the entire strategy. Understanding system dynamics enables finding high-leverage interventions rather than optimizing locally.

**Violation Consequences**:
- Optimizing balancing loops instead of removing them
- Missing compounding opportunities
- Local optimizations that hurt global performance
- Teams working harder instead of working on the system
- Capability stagnation despite effort
...
**Status**: MANDATORY
**Applies To**: All components, CI/CD pipelines, container builds

Dependencies MUST be managed securely and automatically with zero friction for developers:

#### Automated Security

- **Dependabot or Renovate** enabled on all repositories
- **Automatic security updates** for critical vulnerabilities
- **Vulnerability scanning** in CI/CD pipeline
- **SBOM generation** for all container images
- Developers should NOT need to think about dependency security—it happens automatically

#### Version Pinning

- **Pin exact versions** in lock files (`go.sum`, `package-lock.json`, `requirements.txt`)
- **Review changelogs** before major version bumps
- **Test thoroughly** after dependency updates

#### Supply Chain Security

- **Verify checksums** for downloaded dependencies
- **Use trusted registries** for container base images
- **Audit transitive dependencies** for known vulnerabilities
- **Sign** release artifacts and container images

#### GitHub Actions Security

- **NEVER use `pull_request_target`** in GitHub Actions workflows
  - Exposes secrets to untrusted code from forks
  - Use `pull_request` event instead
  - If secrets needed for fork PRs, use workflow_run pattern with explicit trust boundaries
- **Pin actions to SHA** not tags (tags can be moved)
- **Minimize secrets exposure** in workflows
- **Audit third-party actions** before use

#### Zero-Friction Developer Experience

- Security tooling MUST NOT slow down development workflow
- Automated fixes preferred over manual remediation
- Clear error messages when security checks fail
- Pre-configured security in project templates

**Rationale**: Security vulnerabilities in dependencies are a leading attack vector. Manual dependency management is error-prone and inconsistent. Automated, zero-friction security ensures protection without burdening developers or slowing delivery.

**Violation Consequences**:
- Known vulnerabilities in production
- Supply chain attacks
- Secret exposure via GitHub Actions exploits
- Manual toil for security updates
- Inconsistent security posture across components
...
- Design for ~1000 active (Running) pods and ~5000 idle pods at scale
- Per-user baseline: ~5 running, ~5 idle, ~50 stopped sessions
- **Proactive idle stopping**: Sessions automatically transition to `desired-phase=Stopped` after configurable idle timeout
- **State hydration**: Stopped sessions preserve state in S3; use `init-hydrate` container to restore on resume
- **State sync**: Running sessions continuously sync state to S3 via `sync.sh` sidecar
- Session phases: `Pending` → `Creating` → `Running` → `Stopping` → `Stopped` (or `Completed`/`Failed`)
- Resource efficiency: Stopped sessions consume no compute; only storage for persisted state
...
**Current Version**: 3.0.0 (RATIFIED)
...
**Primary Documents**:
- `/CLAUDE.md` - Agent development patterns and examples (symlink to AGENTS.md)
- `/AGENTS.md` - Agent development patterns and examples (primary file)
...
- `/components/runners/*/README.md` - Runner documentation
- `/components/public-api/README.md` - Public API gateway
- `/components/ambient-api-server/README.md` - Ambient API Server (TRex.AI)
- `/components/ambient-sdk/README.md` - SDKs (Go, Python, TypeScript)
- `/components/ambient-cli/README.md` - CLI (acpctl)
- `/components/open-webui-llm/README.md` - Open WebUI integration

**Extended Documentation**:
- `/docs/src/content/docs/` - Astro Starlight documentation for deployment, operations, integrations
- `/docs/internal/` - Internal design docs, proposals, and ADRs
...
### Version 3.0.0 (2026-03-04) - RATIFIED

**Status**: RATIFIED - Multi-Provider & System Dynamics Release

**Major Changes**:
- **Multi-provider AI support**: Platform now supports Claude, Gemini, and future AI providers
- **New Principle XI: System Dynamics & Control Theory**: Design systems around reinforcing/balancing loops
- **New Principle XII: Dependency Management**: Automated, secure, zero-friction dependency handling
- **Pluggable interfaces**: REST API, SDK, MCP, Web UI as first-class client types
- **JobSets support**: Added to Kubernetes-native patterns for coordinated workloads
- **Session lifecycle management**: Documented capacity planning (~1000 active, ~5000 idle pods)
- **State hydration**: Documented session state persistence and restoration via S3
- **Documentation migration**: Updated references from MkDocs to Astro Starlight
- **AGENTS.md**: Added as primary agent config file (CLAUDE.md as symlink)
- **GitHub Actions security**: NEVER use `pull_request_target`
- **New components documented**: Ambient API Server, Ambient SDK, Ambient CLI (acpctl), Open WebUI
- **Upcoming components**: MCP Server, Mobile App

**Rationale**: Platform evolution to support multiple AI providers requires architectural changes. System dynamics principles formalize the platform's design philosophy. Dependency management ensures security at scale.
...
**Ratified**: 2026-03-04
**Last Amended**: 2026-03-04
**Next Review**: 2026-06-04 (quarterly review recommended)
