...
**Shared types:** `components/ambient-sdk/go-sdk/types`
...
├── tools/
│   ├── sessions.go      # Session tool handlers
│   ├── agents.go        # Agent tool handlers
│   └── projects.go      # Project tool handlers
├── mention/
│   └── resolve.go       # @mention parsing and agent resolution
├── Dockerfile
└── go.mod               # module: github.com/ambient-code/platform/components/mcp-server
```
...
This annotation is set on the Session resource at creation time. The operator reads it and injects the `mcp-server` container into the runner Job pod.
...
│     reads config → connects to mcp-server via stdio
│
