...
erDiagram
    ...
    Session {
        string  ID PK
        string  project_id FK
        string  agent_id FK "nullable — future: agent ignition"
    ...
    %% ── SessionMessage (all AG-UI protocol events) ────────────────────────────

    SessionMessage {
        string ID PK
        string session_id FK
        int    seq
        string event_type "user | MESSAGES_SNAPSHOT | RUN_FINISHED | TEXT_MESSAGE_* | TOOL_CALL_* | etc."
        string payload    "JSON — AG-UI event payload or raw text for user turns"
        time   created_at
    }
    ...
    Session         ||--o{ SessionMessage   : "conversation"
...
| `GET /sessions/{id}/messages` | `acpctl session messages <id>` | Human conversation snapshot |
| `GET /sessions/{id}/messages` | `acpctl session messages <id> -f` | Live stream (SSE) |
| `POST /sessions/{id}/messages` | `acpctl session send <id> <msg>` | Send user message |
| `GET /sessions/{id}/ag-ui/events` | `acpctl session ag-ui <id>` | _(planned)_ Raw AG-UI event snapshot |
| `GET /sessions/{id}/ag-ui/events` | `acpctl session ag-ui <id> -f` | _(planned)_ Live AG-UI event stream |
...
POST   /api/ambient/v1/sessions                           create session
PATCH  /api/ambient/v1/sessions/{id}                      update session
PATCH  /api/ambient/v1/sessions/{id}/status               update session status (CP use)
POST   /api/ambient/v1/sessions/{id}/start                start session
POST   /api/ambient/v1/sessions/{id}/stop                 stop session
DELETE /api/ambient/v1/sessions/{id}                      delete session

GET    /api/ambient/v1/sessions/{id}/messages             list all stored AG-UI events (JSON or SSE via Accept header)
POST   /api/ambient/v1/sessions/{id}/messages             push any event type (internal/testing)
GET    /api/ambient/v1/sessions/{id}/ag_ui                SSE stream of all events (real-time feed for frontends)
POST   /api/ambient/v1/sessions/{id}/ag_ui                send user turn (always event_type=user; runner watches this)
...
**Note:** Sessions are currently directly creatable via `POST /sessions`. Agent ignition (`POST /agents/{id}/ignite`) is planned but not yet implemented.

#### SessionMessage Storage

All AG-UI events from the runner are stored in a single `SessionMessage` table. The runner pushes `MESSAGES_SNAPSHOT` (full conversation JSON after each turn) and `RUN_FINISHED` via gRPC. `user` events are stored when a user message is sent via the REST API.

| Event type stored | Written by | Purpose |
|---|---|---|
| `user` | API server on `POST /sessions/{id}/messages` or `POST /sessions/{id}/ag_ui` | User turn input |
| `MESSAGES_SNAPSHOT` | Runner via gRPC push | Full conversation state after each Claude turn |
| `RUN_FINISHED` | Runner via gRPC push | Marks end of inference run |
| Other AG-UI types | Runner via gRPC push (passthrough) | Tool calls, text deltas, etc. |

#### AG-UI Event Types (stored in SessionMessage)

| Event type | Key field | Description |
|---|---|---|
| `user` | `payload` | Inbound user message (raw text) |
| `MESSAGES_SNAPSHOT` | `payload` | Full conversation JSON after each Claude turn (persistent) |
| `RUN_FINISHED` | `payload` | Inference run complete — includes thread/run IDs (persistent) |
| `TEXT_MESSAGE_START` | `role` | Assistant begins a text reply |
| `TEXT_MESSAGE_CONTENT` | `delta` | Incremental text chunk |
| `TEXT_MESSAGE_END` | `message_id` | Text reply complete |
| `TOOL_CALL_START` | `tool_call_name` | Tool invocation begins |
| `TOOL_CALL_ARGS` | `delta` | Incremental tool argument chunk |
| `TOOL_CALL_END` | `tool_call_id` | Tool invocation complete |
| `TOOL_CALL_RESULT` | `content` | Tool output returned to model |
| `RUN_STARTED` | `run_id` | Inference run begins |
| `RUN_ERROR` | `message` | Inference run failed |

**Note:** The runner currently persists only `MESSAGES_SNAPSHOT` and `RUN_FINISHED` via gRPC push. Other event types pass through the SSE stream (`GET /sessions/{id}/ag_ui`) but are not guaranteed to be stored.
...
| Unified `SessionMessage` table | All AG-UI events (user turns, `MESSAGES_SNAPSHOT`, `RUN_FINISHED`, and passthrough events) are stored in a single table. The runner persists conversation state as `MESSAGES_SNAPSHOT` after each turn rather than individual `TEXT_MESSAGE_CONTENT` deltas. `AGUIEvent` as a separate entity is not implemented. |
