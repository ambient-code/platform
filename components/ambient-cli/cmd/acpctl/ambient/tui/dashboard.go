...
func (m *Model) updateSessionTileContent(sessionID string) {
	if m.sessionTileContent == nil {
		m.rebuildMain()
		return
	}
	bounds, ok := m.sessionTileContent[sessionID]
	if !ok {
		m.rebuildMain()
		return
	}
	contentStart, contentEnd := bounds[0], bounds[1]
	if contentStart < 0 || contentEnd > len(m.mainLines) || contentStart >= contentEnd {
		m.rebuildMain()
		return
	}
	...
	msgLines := contentEnd - contentStart
	if msgLines < 1 {
		return
	}

	msgs := m.sessionMsgs[sessionID]
	contentLines := renderTileMessages(msgs, innerW-2, msgLines)
	borderStyle := sessionPhaseStyle(sess.Phase)

	for i, l := range contentLines {
		if contentStart+i >= contentEnd {
			break
		}
		m.mainLines[contentStart+i] = borderStyle.Render("│") + " " + padStyled(l, innerW-1) + borderStyle.Render("│")
	}
}

func sessionPhaseStyle(phase string) lipgloss.Style {
	switch phase {
...
func buildAgentLines(d DashData) []string {
	...
	for _, a := range d.Agents {
		...
		line := styleBlue.Render(col(a.Name, 24)) +
			...
			col(fmt.Sprintf("v%d", a.Version), 10) +
...
func renderSessionDetail(sess sdktypes.Session, msgs []sdktypes.SessionMessage) []string {
	phase := sess.Phase
	if phase == "" {
		phase = "unknown"
	}
	...
	for _, msg := range msgs {
		...
		evStyle := eventTypeStyle(msg.EventType)
		seqStr := styleDim.Render(fmt.Sprintf("#%-4d", msg.Seq))
		lines = append(lines, "  "+seqStr+"  "+ts+"  "+evStyle.Render(col(msg.EventType, 22))+"  "+display)
	}
	return lines
}
...
func renderAgentDetail(agent sdktypes.Agent) []string {
	return []string{
		...
		"  " + col("Version:", 20) + fmt.Sprintf("v%d", agent.Version),
		"",
