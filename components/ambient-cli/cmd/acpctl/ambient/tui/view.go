...
func (m *Model) renderBreadcrumb() string {
	...
	if m.panelFocus && !m.detailMode {
		...
		return crumb + sep + styleOrange.Render("panel") + styleDim.Render("  ↑↓/jk navigate  ·  "+enterHint+extra+"  ·  q back")
	}
	if m.detailMode && m.detailSelectable {
		return crumb + sep + styleDim.Render("panel") + sep + styleOrange.Render(m.detailTitle) + styleDim.Render("  ↑↓/jk navigate  ·  Enter open  ·  q back")
	}
	if m.detailMode && m.detailSplit {
		...
		return crumb + sep + styleDim.Render("panel") + sep + styleOrange.Render(m.detailTitle) + styleDim.Render("  ["+panel+"]  ↑↓/jk scroll  ·  Tab switch panel  ·  q back")
	}
	if m.detailMode {
		return crumb + sep + styleDim.Render("panel") + sep + styleOrange.Render(m.detailTitle) + styleDim.Render("  ↑↓/jk scroll  ·  q back")
	}
...
func (m *Model) renderFooter() string {
	...
	if m.agentEditMode {
		...
		return styleOrange.Render("✎") + " " + styleBold.Render("Editing: "+m.agentEditAgent.Name) + dirty + styleDim.Render("  Enter save  ·  Esc unfocus") + status
	}
	if m.composeMode {
