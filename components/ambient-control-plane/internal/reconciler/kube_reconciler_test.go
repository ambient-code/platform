...
func TestBuildCredentialSidecars_NoCredentials(t *testing.T) {
	...
	sidecars, urls := r.buildCredentialSidecars("test-session", map[string]string{})
	if len(sidecars) != 0 {
...
func TestBuildCredentialSidecars_NoImageConfigured(t *testing.T) {
	...
	sidecars, urls := r.buildCredentialSidecars("test-session", credentialIDs)
	if len(sidecars) != 0 {
...
func TestBuildCredentialSidecars_LocalImagePullPolicy(t *testing.T) {
	...
	credentialIDs := map[string]string{"github": "cred-123"}
	sidecars, _ := r.buildCredentialSidecars("test-session", credentialIDs)
