...
func Load() (*ControlPlaneConfig, error) {
	cfg := &ControlPlaneConfig{
		...
		APIClientID:           readFileOrEnv("AMBIENT_API_CLIENT_ID_FILE", "AMBIENT_API_CLIENT_ID"),
		APIClientSecret:       readFileOrEnv("AMBIENT_API_CLIENT_SECRET_FILE", "AMBIENT_API_CLIENT_SECRET"),
...
func readFileOrEnv(fileEnvKey, directEnvKey string) string {
	if path := os.Getenv(fileEnvKey); path != "" {
		if data, err := os.ReadFile(path); err == nil {
			return strings.TrimSpace(string(data))
		}
	}
	return os.Getenv(directEnvKey)
}
