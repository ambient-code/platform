...
func (d *inboxMessageDaoMock) UnreadByProjectAgentID(ctx context.Context, projectAgentID string) (InboxMessageList, error) {
	var result InboxMessageList
	for _, m := range d.inboxMessages {
		if m.ProjectAgentId == projectAgentID && (m.Read == nil || !*m.Read) {
