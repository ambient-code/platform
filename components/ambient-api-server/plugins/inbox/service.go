...
type InboxMessageService interface {
	...
	UnreadByProjectAgentID(ctx context.Context, projectAgentID string) (InboxMessageList, *errors.ServiceError)
...
func (s *sqlInboxMessageService) UnreadByProjectAgentID(ctx context.Context, projectAgentID string) (InboxMessageList, *errors.ServiceError) {
	messages, err := s.inboxMessageDao.UnreadByProjectAgentID(ctx, projectAgentID)
	if err != nil {
		return nil, errors.GeneralError("Unable to get unread inbox messages for project-agent %s: %s", projectAgentID, err)
	}
	return messages, nil
}
