...
type InboxMessageDao interface {
	...
	UnreadByProjectAgentID(ctx context.Context, projectAgentID string) (InboxMessageList, error)
}
...
func (d *sqlInboxMessageDao) UnreadByProjectAgentID(ctx context.Context, projectAgentID string) (InboxMessageList, error) {
	g2 := (*d.sessionFactory).New(ctx)
	messages := InboxMessageList{}
	if err := g2.Where("project_agent_id = ? AND (read IS NULL OR read = false)", projectAgentID).Order("created_at ASC").Find(&messages).Error; err != nil {
		return nil, err
	}
	return messages, nil
}
