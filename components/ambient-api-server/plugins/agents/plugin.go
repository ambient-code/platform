...
func init() {
	...
	pkgserver.RegisterRoutes("agents", func(apiV1Router *mux.Router, services pkgserver.ServicesInterface, authMiddleware environments.JWTMiddleware, authzMiddleware auth.AuthorizationMiddleware) {
		...
		projectsRouter.HandleFunc("/{id}/agents/{pa_id}/start", startHandler.Start).Methods(http.MethodPost)
		projectsRouter.HandleFunc("/{id}/agents/{pa_id}/start", startHandler.IgnitionPreview).Methods(http.MethodGet)
