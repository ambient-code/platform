...
func init() {
	...
	pkgserver.RegisterRoutes("projectDocuments", func(apiV1Router *mux.Router, services pkgserver.ServicesInterface, authMiddleware environments.JWTMiddleware, authzMiddleware auth.AuthorizationMiddleware) {
		...
		projectDocsSubRouter := apiV1Router.PathPrefix("/projects/{project_id}/documents").Subrouter()
		projectDocsSubRouter.HandleFunc("", projectDocumentHandler.ListByProject).Methods(http.MethodGet)
		projectDocsSubRouter.HandleFunc("/{slug}", projectDocumentHandler.UpsertBySlug).Methods(http.MethodPut)
		projectDocsSubRouter.Use(authMiddleware.AuthenticateAccountJWT)
		projectDocsSubRouter.Use(authzMiddleware.AuthorizeApi)
	})
