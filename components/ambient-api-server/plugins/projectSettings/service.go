...
func (s *sqlProjectSettingsService) AllByProjectId(ctx context.Context, projectId string) (ProjectSettingsList, *errors.ServiceError) {
	list, err := s.dao.AllByProjectId(ctx, projectId)
	if err != nil {
		return nil, errors.GeneralError("Unable to get project settings for project %s: %s", projectId, err)
	}
	return list, nil
}
