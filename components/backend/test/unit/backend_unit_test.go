...
var _ = BeforeSuite(func() {
	...
	Expect(err).NotTo(HaveOccurred(), fmt.Sprintf("Error setting environment to local"))
	...
	if *config.GoTestMode {
		...
		Expect(err).NotTo(HaveOccurred(), fmt.Sprintf("Error setting GO_TEST from config"))
	}

	if *config.DisableAuth {
		err = os.Setenv("DISABLE_AUTH", "true")
		Expect(err).NotTo(HaveOccurred(), fmt.Sprintf("Error setting DISABLE_AUTH from config"))
	}
...
func TestBackend(t *testing.T) {
	...
	os.Setenv(test_constants.ENV_DISABLE_AUTH, "true")
	os.Setenv(test_constants.ENV_GO_TEST, "true")
