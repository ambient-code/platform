...
var _ = Describe("Repository Seeding Handler", Label(test_constants.LabelUnit, test_constants.LabelHandlers, test_constants.LabelRepoSeed), func() {
	...
	BeforeEach(func() {
		...
		// Override GetK8sClientsFunc to return fake clients based on auth header
		// This is the only mutable function - all client selection functions now call this
		...
		handlers.SetGetK8sClientsFunc(func(c *gin.Context) (kubernetes.Interface, dynamic.Interface) {
			// Check if auth should be bypassed (for backward compatibility with DISABLE_AUTH)
		...
		handlers.GetGitHubTokenRepo = func(ctx context.Context, k8s kubernetes.Interface, dyn dynamic.Interface, project, userID string) (string, error) {
	...
	Context("Repository Structure Detection", func() {
		Describe("DetectMissingStructure", func() {
			It("Should detect missing .claude directory", func() {
				...
				// Act
				status, err := handlers.DetectMissingStructure(ctx, testRepoDir)
		...
		Describe("SeedRepository", func() {
			It("Should create .claude directory structure successfully", func() {
				...
				// Act
				response, err := handlers.SeedRepository(ctx, testRepoDir, "https://github.com/test/repo.git", "main", "test@example.com", "Test User")
				Expect(err).NotTo(HaveOccurred())
	...
	Context("HTTP Endpoints", func() {
		Describe("GetRepoSeedStatus", func() {
			It("Should require repo parameter", func() {
				...
				// Act
				handlers.GetRepoSeedStatus(context)
		...
		Describe("SeedRepositoryEndpoint", func() {
			It("Should require valid JSON body", func() {
				...
				// Act
				handlers.SeedRepositoryEndpoint(context)
	...
	Context("Data Structure Validation", func() {
		...
		Describe("SeedResponse", func() {
			It("Should track seeded directories and files", func() {
				// Arrange
				response := handlers.SeedResponse{
					Success:       true,
	...
	Context("Template Validation", func() {
		It("Should have required Claude templates", func() {
			...
			Expect(handlers.ClaudeTemplates).To(HaveKey(".claude/README.md"))
			Expect(handlers.ClaudeTemplates).To(HaveKey(".claude/commands/README.md"))
			Expect(handlers.ClaudeTemplates).To(HaveKey(".claude/settings.local.json"))
			Expect(handlers.ClaudeTemplates).To(HaveKey(".claude/.gitignore"))

			// Check template content is not empty
			for templatePath, content := range handlers.ClaudeTemplates {
		...
		It("Should have valid JSON template for settings", func() {
			settingsTemplate := handlers.ClaudeTemplates[".claude/settings.local.json"]
