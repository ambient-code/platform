...
var _ = Describe("Sessions Handler", Label(test_constants.LabelUnit, test_constants.LabelHandlers, test_constants.LabelSessions), func() {
	...
	BeforeEach(func() {
		...
		// Set up test environment for authentication bypass
		os.Setenv("NAMESPACE", "ambient-code-test")
		// GO_TEST and DISABLE_AUTH are true by default from config
		...
		// For sessions tests, we need to set all the package-level K8s client variables
		// Different handlers use different client variables:
		// - sessions.go uses handlers.K8sClient for provisionRunnerTokenForSession
		// - middleware.go uses handlers.K8sClientMw for service account operations
		// - projects.go uses handlers.K8sClientProjects
		// We'll use the server.K8sClient that was set up by SetupHandlerDependencies
		// Note: Setting K8sClient to nil to skip runner token provisioning in tests
		// since it requires a concrete *kubernetes.Clientset type, not the interface
		handlers.K8sClient = nil
		handlers.K8sClientMw = k8sUtils.K8sClient
		// Note: K8sClientProjects and DynamicClientProjects are set by SetupHandlerDependencies
	})
	...
	Describe("ListSessions", func() {
		...
		Context("When project has sessions", func() {
			BeforeEach(func() {
				...
				list, err := handlers.DynamicClientProjects.Resource(gvr).Namespace(testNamespace).List(context.Background(), v1.ListOptions{})
				if err != nil {
	...
	Describe("CreateSession", func() {
		Context("When creating a valid session", func() {
			It("Should create session with required fields", func() {
				...
				// Act
				handlers.CreateSession(context)
	...
	Describe("DeleteSession", func() {
		...
		Context("When deleting existing session", func() {
			It("Should delete session successfully", func() {
				...
				// Act
				handlers.DeleteSession(context)
