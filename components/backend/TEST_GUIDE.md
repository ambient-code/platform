...
│   └── test_suite.go       # Main test suite setup
...
2. **Shared Utilities**: Common test logic in `test/` directory (avoid duplication)
3. **Import Pattern**: Test packages import utilities with `"ambient-code-backend/test/..."`
...
token, saName, err := httpUtils.SetValidTestToken(k8sUtils, "namespace", []string{"get", "list", "create"}, "agenticsessions", "")
Expect(err).NotTo(HaveOccurred())
