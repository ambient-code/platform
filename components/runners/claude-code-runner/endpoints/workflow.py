...
async def change_workflow(request: Request):
    ...
    async with state._workflow_change_lock:
        ...
        state._platform_ready = False
        state._first_run = True
        state._adapter_dirty = True

        logger.info("Workflow updated, adapter will reinitialize on next run")
        asyncio.create_task(_trigger_workflow_greeting(git_url, branch, path))
...
async def _trigger_workflow_greeting(git_url: str, branch: str, path: str):
    ...
    try:
        ...
        session_id = state.context.session_id if state.context else "unknown"
