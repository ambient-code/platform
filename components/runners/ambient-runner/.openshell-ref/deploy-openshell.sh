...
NAMESPACE="${NAMESPACE:-ambient-openshell}"
...
usage() {
  echo "Usage: $0"
  echo ""
  echo "Deploys Ambient with OpenShell integration into namespace ${NAMESPACE}."
  echo ""
  echo "Environment variables:"
  echo "  NAMESPACE          Target namespace (default: ambient-openshell)"
  echo "  REGISTRY           Image registry prefix (default: quay.io/ambient_code)"
  echo "  IMAGE_TAG          Image tag (default: openshell-v1)"
...
echo "==> OpenShell Integration Deployment"
...
if [[ -n "$VERTEX_SOURCE_NS" ]]; then
  ...
  if [[ -z "$VERTEX_PROJECT_ID" ]]; then
    ...
    echo "    Auto-detected VERTEX_PROJECT_ID=$VERTEX_PROJECT_ID"
  fi
...
echo "==> Step 2b: Binding openshell-sandbox SCC to control-plane SA"
$CLI adm policy add-scc-to-user openshell-sandbox \
  "system:serviceaccount:${NAMESPACE}:${SA_NAME}" || true

echo "==> Step 3: Creating OpenShell policy ConfigMap"
...
data:
  policy.rego: |
$(sed 's/^/    /' "${SCRIPT_DIR}/policy.rego")
  policy.yaml: |
$(sed 's/^/    /' "${SCRIPT_DIR}/policy.yaml")
EOF

echo "==> Step 4: Deploying api-server-db (PostgreSQL)"
...
echo "==> Step 4b: Waiting for PostgreSQL to be ready"
...
echo "==> Step 5: Deploying api-server (development mode — no JWT)"
...
echo "==> Step 6: Deploying control-plane (with OpenShell enabled)"
    ...
    spec:
      ...
      - name: ambient-control-plane
        ...
        - name: OPENSHELL_ENABLED
          value: "true"
        - name: OPENSHELL_POLICY_CONFIGMAP
          value: "openshell-policy"
        volumeMounts:
...
echo ""
echo "==> Ambient with OpenShell installed successfully in $NAMESPACE"
echo "    API server: https://${API_HOST}"
echo "    Image tag:  $IMAGE_TAG"
echo "    OpenShell:  ENABLED"
echo ""
echo "    Login:  acpctl login --url https://${API_HOST}"
echo ""
echo "    Teardown:"
echo "      oc delete namespace $NAMESPACE"
echo "      oc delete clusterrole ambient-control-plane-${NAMESPACE}"
echo "      oc delete clusterrolebinding ambient-control-plane-${NAMESPACE}"
