...
**Tests**: Required for this security-sensitive integration (auth, SSRF protection, and per-user secret isolation). Automated tests are included alongside implementation tasks.
...
- [x] T014 [US2] Add Gerrit config generation to `components/runners/ambient-runner/ambient_runner/bridges/claude/mcp.py` — implement `generate_gerrit_config(instances)` that: (1) creates `/tmp/gerrit-mcp/` directory, (2) aggregates gitcookies lines from all `git_cookies` instances into a single `/tmp/gerrit-mcp/.gitcookies` file (concatenated, not overwritten per instance), (3) builds `gerrit_config.json` with `gerrit_hosts` array mapping each instance to the Gerrit MCP server's native config format (name, external_url, authentication with type/username/auth_token or gitcookies_path), (4) writes config to `/tmp/gerrit-mcp/gerrit_config.json`, (5) sets `GERRIT_CONFIG_PATH` env var; call this from `build_mcp_servers()` before loading MCP config
